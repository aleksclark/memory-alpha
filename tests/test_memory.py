...
async def test_querying_by_level(sample_store_params, level, expected_snippet):
    ...
    for chunk in result["chunks"]:
        assert chunk["level"] == level, (
            f"Received chunk with level '{chunk['level']}' when requesting '{level}'"
        )
...
async def test_token_limiting(sample_store_params):
    ...
    assert len(result_low["chunks"]) <= len(result_high["chunks"]), (
        "Token limiting didn't reduce chunk count"
    )
...
async def test_multiple_stores_affects_importance(sample_store_params):
    ...
    assert "def add(a, b)" in first_chunk["context"], (
        "The duplicated chunk was not prioritized"
    )
